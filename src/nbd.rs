...
/*async fn transmission_phase(
    ...
    loop {
        ...
        println!("CMD: {}, offset: {}, length: {}", cmd, offset, length);
        
        let error: u32 = match cmd {
            NBD_CMD_READ => {
                let mut buf = vec![0u8; length as usize];
                let mut fs = fs.lock().await;
                let bytes_read = fs.block_read(&zvol_name, offset, &mut buf).await;
                ...
                if bytes_read != buf.len() {
                    println!("Short read: {} vs {}", bytes_read, buf.len());
                    buf.resize(bytes_read, 0);
                }
                
                stream.write_u32(NBD_REPLY_MAGIC).await?;
                stream.write_u32(0).await?; // error
                stream.write_u64(handle).await?;
                stream.write_all(&buf).await?;
                continue;
            }
            NBD_CMD_WRITE => {
                ...
                if bytes_written != buf.len() {
                    println!("Short write: {} vs {}", bytes_written, buf.len());
                    5 // EIO
                } else {
                    0
                }
