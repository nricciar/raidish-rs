...
use crate::fs::{FileSystem,FileSystemError,BLOCK_PAYLOAD_SIZE,SPACEMAP_LOG_BLOCKS_PER_METASLAB};
...
impl<D: BlockDevice> FileSystem<D> {
    pub async fn free(&mut self, start: u64, len: u64) -> Result<(), FileSystemError> {
    ...
    async fn free_in_metaslab(&mut self, ms_idx: usize, start: u64, len: u64) -> Result<(), FileSystemError> {
    ...
    pub async fn allocate_blocks_stripe_aligned(&mut self, blocks_needed: u64) -> Result<Vec<Extent>, FileSystemError> {
    ...
    async fn compact_spacemap_log(&mut self, ms_idx: usize) -> Result<(), FileSystemError> {
    ...
    /*async fn compact_spacemap_log(&mut self, ms_idx: usize) -> Result<(), FileSystemError> {
        ...
        for (&start, &len) in &free_extents {
            let entry = SpaceMapEntry::Free { start, len };
            self.write_block_checked_txg(ms.write_cursor, &entry).await?;
            ms.write_cursor += 1;
        ...
        ms.header.spacemap_blocks = (ms.write_cursor - ms.header.spacemap_start) as u32;
        self.write_block_checked_txg(ms.header_block, &ms.header).await?;
    ...
    pub async fn find_orphaned_blocks(&self) -> Result<Vec<(u64, u64)>, FileSystemError> {
        let uber =
            self.uberblock().ok_or(FileSystemError::NotFormatted)?;
