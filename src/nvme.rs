...
impl ControllerState {
    ...
    fn set_cc(&mut self, value: u32) {
        ...
        println!("    set_cc: old_en={}, new_en={}, connected={}", old_en, new_en, self.connected);
...
impl NvmeTcpServer {
    ...
    async fn handle_connection(
        ...
        println!("[Conn {}] NVMe-TCP connection initialized for device '{}' ({} bytes)", 
        ...
        loop {
            ...
            match read_result {
                Ok(Ok(header)) => {
                    ...
                    match pdu_type {
                        NVME_TCP_PDU_TYPE_CMD => {
                            Self::handle_command(&mut socket, header, &fs, &device_name, device_size, &mut ctrl_state, conn_id).await?;
                        }
                ...
                Err(_) => {
                    println!("[Conn {}] Timeout waiting for PDU - client appears idle", conn_id);
                    break;
                }
            }
        }
    ...
    async fn perform_connection_init(socket: &mut TcpStream) -> std::io::Result<()> {
        ...
        let ic_req = unsafe { 
            std::ptr::read(ic_req_buf.as_ptr() as *const NvmeTcpIcReq) 
        };

        println!("Received IC Request: pfv={}, hpda={}", 
                 u16::from_le(ic_req.pfv), ic_req.hpda);
    ...
    async fn read_pdu_header(socket: &mut TcpStream) -> std::io::Result<NvmeTcpPduHeader> {
        ...
        println!("Read PDU header: type=0x{:02x}, flags=0x{:02x}, hlen={}, pdo={}, plen={}", 
    ...
    async fn handle_command(
        ...
        if remaining > 0 {
            ...
            println!("[Conn {}] Consumed {} bytes of extra PDU data", conn_id, remaining);
        }

        let nsid = u32::from_le(cmd.nsid);
        
        println!("[Conn {}] Received command: opcode=0x{:02x}, cid={}, nsid={}", conn_id, opcode, cid, nsid);
    ...
    async fn handle_identify(
        ...
        match cns {
            0x01 => {
                ...
                println!("    -> CNTLID: 1");
                println!("    -> SUBNQN: {}", String::from_utf8_lossy(&subnqn[..subnqn_len]));
        ...
        let c2h_bytes = unsafe {
            std::slice::from_raw_parts(&c2h_header as *const _ as *const u8, hlen)
        };
    ...
    async fn handle_read(
        ...
        println!("Read: slba={}, nlb={}, offset={}, length={}", slba, nlb, offset, length);

        let mut buffer = vec![0u8; length as usize];
        
        {
            let mut fs_lock = fs.lock().await;
            let bytes_read = fs_lock.block_read(device_name, offset, &mut buffer).await.unwrap();
    ...
    async fn handle_h2c_data(
        ...
        println!("H2C Data: cccid={}, datao={}, datal={}", cccid, datao, datal);
        ...
        {
            let mut fs_lock = fs.lock().await;
            fs_lock.block_write(device_name, offset, &data).await.unwrap();
        }
    ...
    async fn handle_fabrics_command(
        ...
        println!("    Fabrics: fctype=0x{:02x}, cdw10=0x{:08x}, cdw11=0x{:08x}", 
                fctype, cdw10, cdw11);
        ...
        match fctype {
            0x00 => { // PROPERTY SET
                ...
                println!("    Property Set: offset=0x{:02x}, value=0x{:x}", offset, value);
                ...
                match offset {
                    ...
                    0x1C | 0x00 | 0x08 => {
                        ...
                        Self::send_completion(socket, u16::from_le(cmd.cid), NVME_SC_INVALID_FIELD, 0).await?;
                    }
                    _ => {
                        println!("    WARNING: Write to unknown offset 0x{:02x}, accepting", offset);
                        Self::send_completion(socket, u16::from_le(cmd.cid), 0, 0).await?;
                    }
                }
            }
            ...
            0x04 => { // PROPERTY GET
                ...
                let value: u64 = match offset {
                    ...
                    _ => {
                        println!("    Property Get: unknown offset 0x{:02x}, returning 0", offset);
                        0
                    }
                };
                
                println!("    Property Get: offset=0x{:02x}, returning 0x{:x}", offset, value);
            ...
            0x01 => { // CONNECT
                ...
                if qid == 0 {
                    ...
                    println!("    Controller now ENABLED and READY (CC=0x{:x}, CSTS=0x{:x})", 
            ...
            0x05 => { // AUTHENTICATION SEND
                ...
                Self::send_completion(socket, u16::from_le(cmd.cid), NVME_SC_INVALID_OPCODE, 0).await?;
            }
            
            0x06 => { // AUTHENTICATION RECV
    ...
    async fn handle_get_log_page(
        ...
        let c2h_header = NvmeTcpC2HData {
            header: NvmeTcpPduHeader {
                ...
                hlen: std::mem::size_of::<NvmeTcpC2HData>() as u8,
                pdo: 0,
                plen: (std::mem::size_of::<NvmeTcpC2HData>() as u32 + log_data.len() as u32).to_le(),
            },
            cccid: cmd.cid,
            rsvd: 0,
            datao: 0,
    ...
    async fn send_completion(
        ...
        socket.flush().await?;
        println!("    Sent completion: CID={}, Status=0x{:x}, Result=0x{:x}", cid, status, result);
        Ok(())
    }
}
